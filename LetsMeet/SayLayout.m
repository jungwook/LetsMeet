...
NSString *const SayElementKindSectionHeader = @"SayElementKindSectionHeader";
NSString *const SayElementKindSectionFooter = @"SayElementKindSectionFooter";

@interface SayLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <SayDelegateLayout> delegate;
...
@implementation SayLayout

/// How many items to be union into a single rectangle
static const NSInteger unionSize = 20;

static CGFloat SayFloorCGFloat(CGFloat value) {
...
- (void)setItemRenderDirection:(SayLayoutItemRenderDirection)itemRenderDirection {
...
- (CGFloat)itemWidthInSectionAtIndex:(NSInteger)section {
    ...
    return SayFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
}
...
- (id <SayDelegateLayout> )delegate {
    return (id <SayDelegateLayout> )self.collectionView.delegate;
}
...
- (void)commonInit {
    ...
    _itemRenderDirection = SayLayoutItemRenderDirectionShortestFirst;
}
...
- (void)prepareLayout
{
    ...
    NSAssert([self.delegate conformsToProtocol:@protocol(SayDelegateLayout)], @"UICollectionView's delegate should conform to SayDelegateLayout protocol");
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        CGFloat width = self.collectionView.bounds.size.width - sectionInset.left - sectionInset.right;
        NSInteger columnCount = [self columnCountForSection:section];
        CGFloat itemWidth = SayFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
        ...
        if (headerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:SayElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
        ...
        for (idx = 0; idx < itemCount; idx++) {
            ...
            if (itemSize.height > 0 && itemSize.width > 0) {
                itemHeight = SayFloorCGFloat(itemSize.height * itemWidth / itemSize.width);
            }
        ...
        if (footerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:SayElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
    ...
    switch (self.itemRenderDirection) {
        case SayLayoutItemRenderDirectionShortestFirst:
            index = [self shortestColumnIndexInSection:section];
            break;
            
        case SayLayoutItemRenderDirectionLeftToRight:
            index = (item % columnCount);
            break;
            
        case SayLayoutItemRenderDirectionRightToLeft:
