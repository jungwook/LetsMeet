...
@protocol ValueTrackingSliderDelegate;
@protocol ValueTrackingSliderDataSource;

@interface ValueTrackingSlider : UISlider
...
@property (weak, nonatomic) id<ValueTrackingSliderDataSource> dataSource;

// delegate is only needed when used with a TableView or CollectionView - see below
@property (weak, nonatomic) id<ValueTrackingSliderDelegate> delegate;
@end



// to supply custom text to the popUpView label, implement <ValueTrackingSliderDataSource>
// the dataSource will be messaged each time the slider value changes
@protocol ValueTrackingSliderDataSource <NSObject>
- (NSString *)slider:(ValueTrackingSlider *)slider stringForValue:(float)value;
@end

// when embedding an ValueTrackingSlider inside a TableView or CollectionView
...
@protocol ValueTrackingSliderDelegate <NSObject>
...
// the recommended technique for use with a tableView is to create a UITableViewCell subclass ↓
 ...
 @interface SliderCell : UITableViewCell <ValueTrackingSliderDelegate>
 @property (weak, nonatomic) IBOutlet ValueTrackingSlider *slider;
 @end
